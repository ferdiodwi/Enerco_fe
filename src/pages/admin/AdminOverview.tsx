...
export default function AdminOverview() {
  ...
  return (
    <div className="space-y-8">
      <div><h1 className="text-2xl font-bold text-white">Dashboard Admin</h1><p className="text-slate-400 mt-1">Ringkasan data sistem EnergEco GlobalChain</p></div>
      ...
      <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ delay: 0.5 }}>
        <Card className="bg-slate-900/70 border-slate-800/60">
          <CardContent className="p-6">
            <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2"><Brain size={20} className="text-purple-400" /> Rekomendasi Terbaru</h2>
              ...
              <div className="space-y-3">
                {data.recent_recommendations.map((rec: any) => (
                  <div key={rec.id} className="flex items-center justify-between p-4 rounded-xl bg-slate-800/40 border border-slate-700/30">
                    ...
                    <Badge variant="outline" className={`capitalize ${rec.status === "approved" ? "border-emerald-500 text-emerald-400" : rec.status === "draft" ? "border-slate-500 text-slate-400" : "border-amber-500 text-amber-400"}`}>{rec.status}</Badge>
                  </div>
                ))}
              </div>
            ) : (
